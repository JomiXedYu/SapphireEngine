...
namespace SapphireEngine 
{
    ...
    bool CheckShaderCompile(const uint32_t& shaderId)
    {
        int isSuccess;
        glGetShaderiv(shaderId, GL_COMPILE_STATUS, &isSuccess);
        return isSuccess;
    }
    string GetShaderCompileErrorInfo(const uint32_t& shaderId)
    {
        char info[512];
        glGetShaderInfoLog(shaderId, 512, nullptr, info);
        return string(info);
    }
    uint32_t CreateShader(const string& shaderName, const uint32_t& shaderType, const string& shaderSource)
    {
        uint32_t shader;
        ...
        if (!CheckShaderCompile(shader)) {
            throw ShaderCompileException(shaderName + _T("  ") + GetShaderCompileErrorInfo(shader));
        }
    ...
    {
        uint32_t id = CreateShader(shaderName, GL_VERTEX_SHADER, shaderSource);
        return Shader(shaderName, id);
    }

    Shader Shader::CreateFragmentShader(const string& shaderName, const string& fragSource)
    {
        uint32_t id = CreateShader(shaderName, GL_FRAGMENT_SHADER, fragSource);
        return Shader(shaderName, id);
    }
